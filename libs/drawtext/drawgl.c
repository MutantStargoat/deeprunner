...
int dtx_gl_setopt(enum dtx_option opt, int val)
{
	...
	case DTX_GL_BLEND:
		use_blend = val;
		break;
	case DTX_GL_ALPHATEST:
		if(val) {
			use_atest = 1;
			atest_thres = val / 255.0f;
		} else {
			use_atest = 0;
			atest_thres = 0;
		}
...
int dtx_gl_getopt(enum dtx_option opt, int *res)
{
	...
	case DTX_GL_BLEND:
		*res = use_blend;
		break;
	case DTX_GL_ALPHATEST:
		*res = use_atest ? (int)(atest_thres * 255.0f) : 0;
		break;
	default:
		return -1;
	}
...
void dtxhack_replace_texture(struct dtx_glyphmap *gmap, unsigned int tex)
{
	gmap->tex = tex;
	gmap->tex_valid = 1;
}
...
static void flush(void)
{
	...
	if(use_atest) {
		...
		glAlphaFunc(GL_GREATER, atest_thres);
	} else {
